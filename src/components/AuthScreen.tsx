...
export default function AuthScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      const { error } = await signUp(email, password, storeName || 'Mi Tienda');
      ...
      <div className="w-full max-w-sm space-y-5">
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Vende directo por WhatsApp</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4">
          ...
          <div className="flex bg-slate-950/60 rounded-2xl p-1 border border-slate-800">
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${
                mode === 'signup' ? 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Crear Cuenta
            </button>
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${
                mode === 'login' ? 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Iniciar Sesión
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {mode === 'signup' && (
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5">Nombre de tu Tienda</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Ej. Boutique Artesanal"
            ...
            {error && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-3 text-xs text-red-300">
                {error}
              </div>
            )}
            ...
            >
              {submitting ? 'Procesando...' : (
                <>
                  {mode === 'signup' ? 'Crear mi Tienda Gratis' : 'Entrar a mi Tienda'}
                  <ArrowRight size={16} />
                </>
              )}
            </button>
          ...
          {mode === 'signup' && (
            <div className="space-y-2 pt-2 border-t border-slate-800">
              <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1.5">
                <Sparkles size={12} className="text-emerald-400" /> Todo lo que incluye tu cuenta:
              </p>
              {[
                'Tu tienda online con catálogo y carrito',
                'Pedidos directos a tu WhatsApp',
                'Código QR para compartir con clientes',
                'Métodos de pago peruanos (Yape, Plin, BCP)',
              ].map((feat, i) => (
